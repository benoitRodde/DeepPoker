...
def hand_strength_estimation(nb_sim, nb_player, cards, community):

    print('Equity calcul for this state')

    # use monte carlo
    convert_cards = gen_cards(cards)

    convert_community_cars = gen_cards(community)
    return estimate_hole_card_win_rate(nb_simulation=nb_sim, nb_player=nb_player, hole_card=convert_cards, community_card=convert_community_cars)

    # action history : round_state["action_histories"][street]


def hand_strength_estimation_with_time(second, nb_player, cards, community):
    ...
    date = datetime.now()
    date_limit = (datetime.now() + timedelta(seconds=second))
    total = 0
    equity = 0

    while date < date_limit:
        equity += estimate_hole_card_win_rate(nb_simulation=1, nb_player=nb_player, hole_card=convert_cards, community_card=convert_community_cars)
        date = datetime.now()
        total += 1
    print(total)
